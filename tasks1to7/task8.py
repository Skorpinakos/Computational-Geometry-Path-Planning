...
class Task7_scene(Scene3D):
    ...
    def reset_mesh(self):
        def translate_meshes_to_pcd_center(object_list,pcd_path="results/pcd_aligned.ply"):
            ...
            # Translate each object in the list
            for obj in object_list:
                translation = [-center_x, -center_y, 0]  # Adjust z if necessary
                if isinstance(obj, o3d.geometry.TriangleMesh):
                    obj.translate(translation, relative=False)
                elif  isinstance(obj, Mesh3D):
                    obj.vertices = (np.asarray(obj.vertices) + np.array(translation)).tolist()
        ...
        translate_meshes_to_pcd_center([self.mesh_aligned_with_holes])
        ...
        translate_meshes_to_pcd_center([self.mesh])
        ...
        translate_meshes_to_pcd_center([self.filtered_pcd])
    ...
    def on_key_press(self, symbol, modifiers):
        ...
        if symbol == Key.P:
            #self.addShape(PointSet3D(np.asarray(o3d.io.read_point_cloud("results/pcd_aligned.ply").points)),"pcd")
            #return
            path_height=1
            ...
            from task8_beta import main
            start = np.array(start)
            end = np.array(end)
            print("Finding path")
            t1=time.time()
            referenced_path_points=main(start,end)
            t2=time.time()
...
def region_growing(pcd, threshold_distance=0.5, max_points=5000, min_points=4):
    points = np.asarray(pcd.points)
